...
class Resource:
    class Variety:
        def __init__(self, filepath, content=None, hash=None):
            ...
            self.name = ".".join(filename.split(".")[0:-2])
            ...
            if isinstance(hash, list):
                ...
                if isfile(filepath):
                    ...
                    raise RuntimeError("File %s provided to create variety of resource is invalid!" % file)

        def get_hash_string(self):
            """Returns the hash of the vile as a string.
            """
            arraystr = numpy.array2string(self.hash.digest(), separator=" ", max_line_width=10000)
            translated_hash = list(arraystr[1:-2].split(" "))
            while '' in translated_hash: translated_hash.remove('')
    ...
    def __str__(self):
        return "Resource<{id}, {extension}, {webpage}>".format(id=self._id, extension=self.extension, webpage=self.webpage)
    ...
    def add_varieties(self, varieties):
        ...
        _logger.debug("%r varieties are %s" % (self, self.varieties))
...
class Run(list):
    ...
    def __repr__(self):
        return "{0} <{1}>".format(self.__class__.__name__, self.get_id())
    ...
    def __contains__(self, val: Resource):
        ...
        for case in self._list:
            ...
            if max_sim < sim:
                max_sim = sim
            if max_sim >= self._COMPARISON_TRESHOLD and case.extension == val.extension and case.webpage == val.webpage:
                break
        return max_sim >= self._COMPARISON_TRESHOLD and case.extension == val.extension and case.webpage == val.webpage
    ...
    def join(self, other, compatible=False):
        ...
        if compatible:
            if not (self.get_id() is None) and not (other.get_id() is None):
                if otherrun.get_id() != selfrun.get_id():
                    joined = False
        ...
        if joined:
            for res in other:
                ...
                if not (index is None):
                    ...
                    if similarity >= self._COMPARISON_TRESHOLD and similarity < 1.0:
                        self._list[index].add_varieties(res.get_varieties())
                        _logger.debug("Joining resources %r and %r" % (self._list[index], res))
                else:
                    self.append(res)
...
class RunCollection(list):
    ...
    def __repr__(self):
        return "{0} <id: {1}, length: {2}>".format(self.__class__.__name__,self.get_id(), len(self._list))
    ...
    def join(self, other, compatible=False):
        """Attempts joining self with other. If compatible is True, they will only joined if they have the same name.
        """
